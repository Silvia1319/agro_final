...
class DominoSlicer:
    def __init__(self, n_slices=5, covariance_type="full", n_pca_components=128, n_mixture_components=25,
                 y_log_likelihood_weight=1, y_hat_log_likelihood_weight=1, max_iter=100, init_params="confusion",
                 confusion_noise=1e-3):

        self.n_slices = n_slices
        # Directly setting attributes on the instance
        ...
        self.pca = PCA(n_components=self.n_pca_components) if self.n_pca_components is not None else None

        # Initialize the Domino Mixture Model
        self.mm = DominoMixture(
            ...
            confusion_noise=self.confusion_noise
        )

    def fit(self, data, embeddings, targets, pred_probs):
        # Assume unpack_args is a defined utility function that processes inputs
        ...
        if hasattr(self.mm, 'y_hat_probs') and hasattr(self.mm, 'y_probs'):
            self.slice_cluster_indices = (
                -np.abs((self.mm.y_hat_probs - self.mm.y_probs).max(axis=1))
            ).argsort()[:self.n_slices]

        return self

    def transform(self, data, embeddings, targets, pred_probs):
        embeddings, targets, pred_probs = unpack_args(data, embeddings, targets, pred_probs)
